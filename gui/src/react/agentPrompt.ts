...
2. MESH: mesh.generate (or the gmsh.* chain above). Coarse mesh → fast iteration.
   For flow around a watertight part (imported STL/STEP or any primitive —
   box/cylinder/cone/sphere/prism/pad), pass maskSolids:true so cells inside the
   part are blanked (a Brinkman penalization makes the solve route flow AROUND
   it); it reports solid/fluid cells.
