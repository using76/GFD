...
- **Phase 4 ✅ (framework) / 🚧 (full port)** — data-driven UI in `src/react/`: `RibbonFromRegistry` (tabs/buttons from the registry), `CommandFormPanel` (inputs from `paramsSchema`), `FeatureTreePanel`, `CoreContext` (React ↔ command-core bridge), `AppV2` shell. Opt-in via `?v2` in `main.tsx`; legacy App stays default. Remaining: author the display/measure/repair/prepare/setup command sets to reach the full 168-feature parity, then retire the legacy store.
