...
- **Phase 4 ✅ (framework + most command sets) / 🚧 (full port)** — data-driven UI in `src/react/`: `RibbonFromRegistry`, `CommandFormPanel` (inputs from `paramsSchema`), `FeatureTreePanel`, `CoreContext`, `AppV2` shell (opt-in via `?v2`). Command sets authored: geometry, sketch, selection, view, **display** (render mode/visibility/section), **measure** (volume/area/CoM/bbox), **repair** (check/fix/stats), mesh, **setup** (models/material/boundaries/solver → feeds calc.run), calc, results, physics. Remaining: the `prepare` set (enclosure/named-selection/defeaturing) for full 168-feature parity, then retire the legacy store.
